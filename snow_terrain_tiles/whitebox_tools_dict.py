...
def wb_tools_in_dict():
    ...
    tool_dict['find_ridges']['help'] = "Identifies potential ridge and peak grid cells. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file. line_thin -- Optional flag indicating whether post-processing line-thinning should be performed."
    ...
    for p in hillshade_params:
        tool_dict['hillshade'][p] = ''
    tool_dict['hillshade']['help'] = "Calculates a hillshade raster from an input DEM. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file. azimuth -- Illumination source azimuth in degrees. altitude -- Illumination source altitude in degrees. zfactor -- Optional multiplier for when the vertical and horizontal units are not the same."

    tool_dict['horizon_angle'] = {}
    horizon_angle_params = ['dem', 'output', 'azimuth', 'max_dist']
    for p in horizon_angle_params:
        tool_dict['horizon_angle'][p] = ''
    tool_dict['horizon_angle']['help'] = "Calculates horizon angle (maximum upwind slope) for each grid cell in an input DEM. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file. azimuth -- Wind azimuth in degrees. max_dist -- Optional maximum search distance (unspecified if none; in xy units)."

    tool_dict['max_downslope_elev_change'] = {}
    max_downslope_elev_change_params = ['dem', 'output']
    for p in max_downslope_elev_change_params:
        tool_dict['max_downslope_elev_change'][p] = ''
    tool_dict['max_downslope_elev_change']['help'] = "Calculates the maximum downslope change in elevation between a grid cell and its eight downslope neighbors. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file."

    tool_dict['min_downslope_elev_change'] = {}
    min_downslope_elev_change_params = ['dem', 'output']
    for p in min_downslope_elev_change_params:
        tool_dict['min_downslope_elev_change'][p] = ''
    tool_dict['min_downslope_elev_change']['help'] = "Calculates the minimum downslope change in elevation between a grid cell and its eight downslope neighbors. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file."
    ...
    tool_dict['plan_curvature'] = {}
    plan_curvature_params = ['dem', 'output', 'zfactor']
    for p in plan_curvature_params:
        tool_dict['plan_curvature'][p] = ''
    tool_dict['plan_curvature']['help'] = "Calculates a plan (contour) curvature raster from an input DEM. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file. zfactor -- Optional multiplier for when the vertical and horizontal units are not the same."
    ...
    tool_dict['average_upslope_flowpath_length']['help'] = "Measures the average length of all upslope flowpaths draining each grid cell. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file."
    ...
    tool_dict['depth_in_sink']['help'] = "Measures depth of sinks (depressions) in a DEM. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file. zero_background -- Flag indicating whether the background value of zero should be used (false)."
    ...
    tool_dict['num_inflowing_neighbours']['help'] = "Computes the number of inflowing neighbours to each cell in an input DEM based on the D8 algorithm. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file."
    ...
    # tool_dict['max_upslope_flowpath_length']['help'] = "Measures the maximum length of all upslope flowpaths draining each grid cell. Keyword arguments: dem -- Input raster DEM file. output -- Output raster file."
